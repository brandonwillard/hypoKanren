...
"
  (conde
    ...
    [(fresh [f e e-let t]
            ;; `let` construct
            ;; TODO: Generalize to any number of binding pairs.
            (== `(let [~f ~e] ~e-let) expr)
            (symbolo f)
            ;; State that `f` is bound to expression `e` in outer-scope `Γ`.
            ;; In a sense, create a lexical scope for the `let` body.
            (!-o (cons `(~f poly ~e ~Γ) Γ) e-let σ)
            (!-o Γ e t))]
    [(fresh [rator rand T]
            ;; [unary] Function application
            ;; TODO: Generalize to any arity.
            (== `(~rator ~rand) expr)
            (!-o Γ rator `(~T → ~σ))
            (!-o Γ rand T))]
    [(fresh [e1 e2]
...
(defn lookupo [Γ x t]
  ...
  (fresh []
    ...
    (conde
      ...
      ;; polymorphic (?) relationship to an expression in a sub-environment (e.g. lexical
      ;; env produced by `let`).
      [(fresh [e Γ^ r]
